...
from reservationApp.models import Booking, Category, Location, Bus, Schedule, Contact
...
@login_required
def mng_contacts(request):
    context['page_title'] = "Manage Contacts"
...
def delete_contact(request):
    ...
    if request.method == 'POST':
        try:
            ...
            name = contact.name
            contact.delete()
            messages.success(request, f'[<b>{name}</b>] contact has been deleted successfully')
            resp['status'] = 'success'
        except Exception as err:
            resp['msg'] = 'contact has failed to delete'
            print(err)
